 ...
 * Selenium waiter
 * 
 * This is the tool-class used for waiting. It can be used to wait for many
 * circumstances (element exist, not exist, be visible, to equal a desired
 * value, etc..
 *
...
{
	// TODO these values need to be removed, and instead accessed from the Config/Execution properties once implemented
	 ...
	 * Constructor for SeleniumWait object
	 * @param browser WebDriver
	 * @param implicitWaitTimeout int value of seconds to wait before timing out
	 */
	...
	/**
	 * Wait for the specified element to be present on the pages DOM.  The first element located with the specified By value is returned
	 ...
	 * Wait for the specified element to be present on the pages DOM.  The first element located with the specified By value is returned
	 * @param by Selector to wait for and return
	 * @param assertFound boolean - true to assert the element was found
	 * @return WebElement
	 */
	 ...
	 * Wait for the specified element to be present on the pages DOM.  The first element located with the specified By value is returned
	 * @param by Selector to wait for, and return
	 * @param timeOutSeconds - the number of seconds to wait for the selector to be displayed before failing
	 * 
	 ...
	 * @param by Selector to wait for, and return
	 * @param timeOutSeconds - the number of seconds to wait for the selector to be displayed before failing
	 * @param printMissingToConsole - boolean: true to print missing selectors to the console
	 * @return WebElement - first one found with by
	 */
	 ...
	 * Wait for the element to not be displayed or visible
	 * @param by Selector to not be displayed or visible
	 * @param waitTime int value of seconds to wait before timing out
	 */
	public void waitForAbsentElement(final By by, final int waitTime)
	{
		Assert.assertTrue(this.fluentWaitInvisible(by, waitTime), String.format(
				"The selector [%s] was still found when it should have disappeared." + 
		System.getProperty("line.separator"), by));
	}
	
	/**
	 * Wait for the element to not be displayed or visible
	 * @param element WebElement to be invisible/not displayed
	 */
	...
	/**
	 * Wait for a selector to present, and then return a list of all WebElements that are located by that selector
	 ...
	 * Wait for a selector to present, and then return a list of all WebElements that are located by that selector
	 * @param by Selector value to wait for
	 * @param timeOutInSeconds - number of seconds 
	 * @return List of WebElements - all web elements found by the specified selector
	 */
	 ...
	 * @param by By selector to examine
	 * @param text String to search for in the text
	 * @param assertFound boolean - true to assert the text matches
	 * @return WebElement - element, null if not found and assert == false
	 */
	public WebElement waitForExactText(final By by, final String text, boolean assertFound)
	{
		...
		if (element == null && assertFound)
		{
			Assert.fail(String.format("The specified selector [%s] couldn't be found." + System.getProperty("line.separator"), by.toString()));
		}
		...
		if (!waitUntilExactText(by, text) && assertFound)
		{
			...
			Assert.assertEquals(displayedText, text, String.format(
					"The observed text [%s] didn't equal the expected text [%s] for selector [%s]." + 
			System.getProperty("line.separator"), displayedText, text, by.toString())); 
		}
	 ...
	 * @param by Selector to examine for the specified text
	 * @param text String value to verify the specified selector contains
	 * @param timeOut int value of seconds to wait before timing out
	 * @return boolean - true if the text was found in the element - else false
	 */
	public boolean waitUntilExactText(final By by, final String text, int timeOut)
	{
		...
		try
		{
			...
			new WebDriverWait(browser, timeOut){}.until(new ExpectedCondition<Boolean>(){
	 ...
	 * @param by Selector to check the containing text
	 * @param text String that should be contained within the selector
	 * @param assertFound boolean to assert the selector is found and contains the text
	 * @return WebElement
	 */
	public WebElement waitForContainsText(final By by, final String text, boolean assertFound)
	{
		...
		{			
			if (element == null)
			{
				Assert.fail(String.format("The selector [%s] wasn't found." + System.getProperty("line.separator"), by.toString()));
			}
			else
			{
	...
	public void waitForAttribute(final By by, final String attribute, final String text)
	{
		if (!waitUntilAttributeTextEquals(by, attribute, text))
		{
			...
			{
				Assert.fail(String.format("The expected text [%s] wasn't contained in the attribute [%s] for selector [%s]." + 
			System.getProperty("line.separator"), text, attribute, by.toString()));
			}
		}
	}
	 ...
	 * @param attribute String value of the attribute to look at on the specified selector
	 * @param text String value of the text to look for in the attribute
	 * @param assertFound - true to assert the element is found, false will return null if it isn't found
	 * @return Webelement of the selector that is found
	 */
	public WebElement waitForAttributeTextEquals(final By by, final String attribute, final String text, boolean assertFound)
	{
		return this.waitForAttributeTextEquals(by, attribute, text, this.implicitWaitTimeout, assertFound);
	}
	 ...
	 * @param attribute String value of the attribute to look at on the specified selector
	 * @param text String value of the text to look for in the attribute
	 * @param timeOut int version of the timeout in seconds
	 * @return true if the attribute with the specified text value is found, else false
	 */
	public boolean waitUntilAttributeTextEquals(final By by, final String attribute, final String text, int timeOut)
	{
	 ...
	 * @return WebElement of the selector that is found
	 */
	public WebElement waitForAttributeTextEquals(final By by, final String attribute, final String text, int timeOut, boolean assertFound)
	{
		...
		if (assertFound)
		{
			Assert.fail(String.format("Selector [%s] couldn't be found." + System.getProperty("line.separator"), by.toString()));
		}
	 ...
	 * @return true if the attribute with the specified text value is found, else false
	 */
	public boolean waitUntilAttributeTextContains(final By by, final String attribute, final String text, int timeOut)
	{		
	 ...
	 * @param attribute String value of the attribute to look at on the specified selector
	 * @param text String value of the text to look for in the attribute
	 * @param assertFound boolean - true to assert the selector/attribute is found. null is returned if false, and not found
	 * @return WebElement of the selector/attribute found
	 */
	public WebElement waitForAttributeTextContains(final By by, final String attribute, final String text, boolean assertFound)
	{
		return this.waitForAttributeTextContains(by, attribute, text, this.implicitWaitTimeout, assertFound);
	}
	 ...
	 * @return WebElement of the selector/attribute found
	 */
	public WebElement waitForAttributeTextContains(final By by, final String attribute, final String text, int timeOut, boolean assertFound)
	{
	 ...
	 * Scroll an element into view, and wait for it to be clickable
	 * @param by Selector to wait for and focus on
	 * @param timeOut int value of seconds to wait for before timing out
	 * @return WebElement
	 */
	 ...
	 * Wait for the specified selector to be clickable
	 * @param by Selector to wait for
	 * @param timeOut integer of seconds to wait before timing out
	 * @return boolean - true if found and clickable, else false
	 */
	 ...
	 * Wait for the element specified by the provided selector to be clickable
	 * @param by Selector to wait for to be clickable
	 * @param timeOutInSeconds int value of the number of seconds to wait before timing out
	 * @return WebElement that is located, or null if none is found
	 */
	 ...
	 * @param by Selector to wait to be removed
	 ...
	 * Fluently wait for an element to be removed
	 * @param by Selector to wait to be removed
	 * @param clearTime int value of how long to wait for the selector to be removed
	 * @return boolean - true if the selector is no longer present, else false
	 */
	public boolean fluentWaitInvisible(final By by, int clearTime)
	{
		...
		{
			System.out.println("Wait for certain text to be present failed for selector: " + by.toString());
			e.printStackTrace();
		}
		return false;
	}
	 ...
	 * @return true if the attribute with the specified text value is found, else false
	 */
	private boolean waitUntilAttribute(final By by, final String attribute, final String text, int timeOut, final boolean contains)
	{		
	...
	private void scrollIntoView(By by)
	{
		...
		try
		{
			((JavascriptExecutor) this.browser).executeScript("arguments[0].scrollIntoView(true);", element);
		}
		catch (Exception e)
		{
			System.out.println("Failed to do JavaScript scroll into view..." + e.getStackTrace() + System.getProperty("line.separator"));
		}
	...
	private boolean fluentWaitStale(WebElement element)
	{
		try
		{
			return new WebDriverWait(this.browser, this.implicitWaitTimeout).pollingEvery(fluentRetryTime, TimeUnit.MILLISECONDS)
	 ...
	 */
	private static boolean attributeMatches(WebDriver browser, By by, String attribute, String text, boolean contains)
	{
		try
		{
			if (contains)
	 ...
	 * @param e Exception to print to the console
	 */
	...
	private boolean doesTextMatch(By by, String text)
	{
		try
		{
			...
			// TODO we probably should include an option to use fuzzy validation here, so it isn't a hard-compare.
	 ...
	 * @param wait int value of seconds to wait
	 */
