...
public class SeleniumUtilitiesUnitTest extends BaseGenericTest {
    ...
    public void testCaptureScreenshotNoAppend() throws Exception {
        ...
        Assert.assertTrue(new File((arrayOfAssociatedFiles[arrayOfAssociatedFiles.length - 1])).exists(),
    ...
    public void testCaptureScreenshotAppend() throws Exception {
        ...
        Assert.assertTrue(new File((arrayOfAssociatedFiles[arrayOfAssociatedFiles.length - 1])).exists(),
                "Checking that expected file path exists");
        Assert.assertTrue(
                new File((arrayOfAssociatedFiles[arrayOfAssociatedFiles.length - 1])).getName().contains(testAppend),
    ...
    public void testCaptureScreenshotCustomDirectoryFileName() throws Exception {
        ...
        String filePath = SeleniumUtilities.captureScreenshot(webDriver, testObject, fileLogger.getDirectory(),
                StringProcessor.safeFormatter("%s - %s", "TestCustomName", dateTime));

        Assert.assertEquals(filePath, Paths.get(StringProcessor.safeFormatter("%s%s - %s%s", fileLogger.getDirectory(),
                "\\TestCustomName", dateTime, ".png")).normalize().toString());
        Assert.assertTrue(new File(filePath).exists(), "Checking that screenshot file exists at expected path.");
    ...
    public void testSavePageSourceCustomDirectoryFileName() throws Exception {
        ...
        String dateTime = DateTimeFormatter.ofPattern("uuuu-MM-dd-HH-mm-ss-SSSS", Locale.getDefault())
                .format(LocalDateTime.now(Clock.systemUTC()));
        String filePath = SeleniumUtilities.savePageSource(webDriver, testObject, fileLogger.getDirectory(),
                StringProcessor.safeFormatter("%s - %s", "TestCustomName", dateTime));

        // Assert File Path returned from Screenshot is the same as expected file path.
        Assert.assertEquals(filePath, Paths.get(StringProcessor.safeFormatter("%s%s - %s%s", fileLogger.getDirectory(),
                "\\TestCustomName", dateTime, ".txt")).normalize().toString());
        Assert.assertTrue(new File(filePath).exists(), "Checking that page source file exists at expected path.");
        String[] arrayOfAssociatedFiles = testObject.getArrayOfAssociatedFiles();
        Assert.assertTrue(new File((arrayOfAssociatedFiles[arrayOfAssociatedFiles.length - 1])).exists(),
                "Checking that expected file path exists");
    ...
    public void testWebElementToWebDriver() throws Exception {
        ...
        WebElement input = UIWaitFactory.getWaitDriver(eventFiringWebDriver).waitForVisibleElement(By.tagName("div"));
