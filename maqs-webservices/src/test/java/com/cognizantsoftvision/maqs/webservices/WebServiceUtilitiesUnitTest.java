...
public class WebServiceUtilitiesUnitTest extends BaseWebServiceTest {
	...
	public void testGetResponseBody() throws Exception {
		HttpResponse<String> response = webServiceDriver.getContent(baseUrl + "/api/String/1", MediaType.PLAIN_TEXT,
				true);
	...
	public void testGetResponseBodyAsObjectFromJson() throws Exception {
		HttpResponse<String> response = webServiceDriver.getContent(baseUrl + "/api/XML_JSON/GetProduct/1",
				MediaType.APP_JSON,
				true);
	...
	/**
	 * Try to get a response body as an object from neither Xml or Jason, this
	 * should throw an error which is caught.
	 * 
	 ...
	 * 
	 * @throws JsonProcessingException      if there is an error in converting Json
	 *                                      to a string
	 * @throws UnsupportedEncodingException if there is an unsupported conversion
	 */
	@Test(groups = TestCategories.WEB_SERVICE)
	public void testCreateStringEntityJson() throws JsonProcessingException, UnsupportedEncodingException {
		String entity = WebServiceUtilities.createStringEntity(this.product, MediaType.APP_JSON);
	 ...
	 */
	@Test(groups = TestCategories.WEB_SERVICE)
	public void testCreateStringEntityXml() throws JsonProcessingException, UnsupportedEncodingException {
		String entity = WebServiceUtilities.createStringEntity(this.product, MediaType.APP_XML);
	...
	public void testSerializeJson() throws JsonProcessingException {
		...
		Assert.assertEquals(expectedJson, actualJson,
				String.format("the json values compared aren't equal, expected was %s while actual was %s",
	...
	public void testSerializeXml() throws JsonProcessingException {
		...
		Assert.assertNotEquals(expectedXml, actualXml, String.format(
				"the xml values compared aren't equal, expected was %s while actual was %s", expectedXml, actualXml));
	}

	/**
	 * Tests that an error occurs when trying to create a string entity with an
	 * unsupported object type.
	 * 
	 * @throws JsonProcessingException      if an error occurs in the object to
	 *                                      string process
	 * @throws UnsupportedEncodingException If there is an illegal or unsupported
	 *                                      exception thrown
	 */
	@Test(expectedExceptions = IllegalArgumentException.class, groups = TestCategories.WEB_SERVICE)
	public void testCreateStringEntityNotJsonOrXml() throws JsonProcessingException, UnsupportedEncodingException {
	 ...
	 * 
	 * @throws JsonProcessingException      if an error is thrown during conversion
	 *                                      to string
	 * @throws UnsupportedEncodingException if there is an unsupported type
	 */
	@Test(groups = TestCategories.WEB_SERVICE)
	public void testCreateStringEntityCustomContentType() throws JsonProcessingException, UnsupportedEncodingException {
		String entity = WebServiceUtilities.createStringEntity(this.product, MediaType.APP_JSON);
